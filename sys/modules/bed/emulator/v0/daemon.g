...
;---------------------------------------------------------------------------------------------
var CURRENT_FILE		= "/sys/modules/bed/emulator/v0/daemon.g"
; Definitions--------------------------------------------------------------------
...
if(exists(global.MODULE_BED) && exists(global.bedTempLastSetTime))
	...
	if((var.useCooldownTimer) && (var.bedTempSet > 0))				
		if((state.upTime - var.bedTempLastSetTime) > var.MAX_WAIT_TIME)
			...
			M98  P"/macros/assert/result.g" R{result} Y"Unable to set the bed temperature to var.MIN_TEMP" F{var.CURRENT_FILE} E10630
			M140 S{var.TURN_OFF_TEMP}
			M98  P"/macros/assert/result.g" R{result} Y"Unable to set the bed temperature to var.OFF_TEMP" F{var.CURRENT_FILE} E10631
