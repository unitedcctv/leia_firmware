; Description: 	
; 	We will emulate the homing of X and Y axes to min.
;------------------------------------------------------------------------------
var CURRENT_FILE = "/sys/modules/axes/emulator/v0/homing/xy.g"
M118 S{"[HOMING] Starting "^var.CURRENT_FILE^" I:"^state.thisInput^" S:"^inputs[state.thisInput].stackDepth}

; Checking global variables and input parameters ------------------------------
; Checking global variables
M98 P"/macros/assert/abort_if.g" R{!exists(global.MODULE_AXES)}  Y{"Missing AXES configuration"} F{var.CURRENT_FILE}  			E10310
M98 P"/macros/assert/abort_if.g" R{!exists(move.axes[0].min)}  	 Y{"Missing X axis proper configuration"} F{var.CURRENT_FILE}  	E10311
M98 P"/macros/assert/abort_if.g" R{!exists(move.axes[1].min)}  	 Y{"Missing Y axis proper configuration"} F{var.CURRENT_FILE}  	E10312
...
G92 X{move.axes[0].min} Y{move.axes[1].min}
M400
